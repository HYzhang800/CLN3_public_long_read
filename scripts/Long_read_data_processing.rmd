...
for (i in 1:length(quantification)) {
  ...
  if (nrow(f[[i]]) != 0) {
    ...
    colnames(f[[i]])[12] <- 'TPM'
    f[[i]]$proportion <- f[[i]]$TPM / sum(f[[i]]$TPM)
...
sample_depth <- quanti_info %>%
  dplyr::select(sample, TPM) %>%
  group_by(sample) %>%
  summarise(across(TPM, sum))

sample_depth <- separate(sample_depth, sample, into = c("replica", "Experiment.accession"), sep = "(?=[E][N])", remove = FALSE, fill = "right")

sample_depth$Biosample.term.name <- quanti_tissue$Biosample.term.name[match(sample_depth$Experiment.accession, quanti_tissue$Experiment.accession)]
...
#write.csv(sample_info, "C:/Users/Haoyu/Desktop/output/manuscript/Tables/CLN3_TPM_summary_in_organs.csv", row.names = F)
...
quanti_info_fusion <- read.csv(file = "E:/Encode_tsv/all/locus_fusion_only.csv")

sample_depth_fusion <- quanti_info_fusion %>%
  dplyr::select(sample, TPM) %>%
  group_by(sample) %>%
  summarise(across(TPM, sum))

sample_depth_fusion <- separate(sample_depth_fusion, sample, into = c("replica", "Experiment.accession"), sep = "(?=[E][N])", remove = FALSE, fill = "right")

sample_depth_fusion$Biosample.term.name <- quanti_tissue$Biosample.term.name[match(sample_depth_fusion$Experiment.accession, quanti_tissue$Experiment.accession)]
...
for (i in 1:length(organs)) {
  tissues <- quanti_tissue %>% filter(organ == organs[i])
  df <- sample_depth_fusion %>% dplyr::filter(Biosample.term.name %in% tissues$Biosample.term.name)
  df$organ <- organs[i]
  organ_depth[[i]] <- df
}

sample_depth_fusion <- bind_rows(organ_depth)

sample_depth_fusion$gene <- "CLN3_readthrough"
...
sample_info_fusion <- sample_info_fusion %>% 
  dplyr::select(
    organs = organ,
    num_samples,
    #tissues,
...
sample_info_fusion
#write.csv(sample_info_fusion, "C:/Users/Haoyu/Desktop/output/manuscript/Tables/fusion_TPM_summary_in_organs.csv", row.names = F)
```
...
quanti_cds <- quanti_info %>% dplyr::select(-c('TPM', 'sample'))
